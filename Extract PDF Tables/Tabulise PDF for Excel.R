library(tabulizer)
fPath <- xlPath
location <- file.path(fPath, "..", "pdf", "Abbreviation-List.pdf")
...
write.csv(states, file.path(fPath, "..", "csv", "States.csv"), row.names = FALSE)
directional <- as.matrix(final[34:41, 1:2], ncol=2)
colnames(directional) <- lapply(as.data.frame(out)[33,1:2], function(x) {gsub("\r", "\\\n", x)})
write.csv(directional, file.path(fPath, "..", "csv", "Directional.csv"), row.names = FALSE)
...
write.csv(address, file.path(fPath, "..", "csv", "Address.csv"), row.names = FALSE)