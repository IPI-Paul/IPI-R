...
# Rcpp::cppFunction() crashes R if used on more than 6 characters. Using logic from my Python App, ipi.duprow.int.matrixi(x,y,z) (z being ncol),  
# matrices, internal function, a for loop, a while loop and rbind.
permTest13 <- function(test) {
  perm13 <- function(x) {
    if (nchar(x) > 6) {
      stop("Any more than six characters will cause R to crash!")
    }
    #require(IPI)
    w <- matrix(seq(strsplit(x, "")[[1]]), ncol = nchar(x))
    rows <- sapply(length(w[1,]), function(z) {for (i in 1:(z-2)) z <- z + z * i; z})
    main <- function(s) {    
      tmp <- w[0,]
      for (i in seq(1:(ncol(s) - 1))) {
        tmp <- matrix(rbind(tmp, append(s[, -i], s[, i])), ncol = ncol(s))
      }
      w <<- matrix(unique(rbind(w, tmp[ipi.duprow.int.matrixi(tmp[ipi.duprow.int.matrixi(tmp, w, ncol(s)),], w, ncol(s)),])), ncol = ncol(s))
      ln <<- ln + 1
    }
    ln <- 1
    while (nrow(w) < rows) {
  ...
  tryCatch({
    desc <- paste("Rcpp::cppFunction() crashes R if used on more than 6 characters. Using logic from my Python App, ipi.duprow.int.matrixi(x,y,z)", 
                  "(z being ncol), matrices, internal function, a for loop, a while loop and rbind.")
    print(desc)
    stm <- system.time(w <- perm13(test))
    cat(paste(capture.output(stm), "\n"))
    chk <- list(unique.rows = nrow(unique(w)), total.rows = nrow(w), characters = ncol(w))
    TestResult[nrow(TestResult),-(c(1:3,11))] <<- c(desc, chk[2], chk[1], chk[3], round(unname(stm)[1:3],2))
  }, 
...
# Using logic from my Python App, ipi.duprow.matrixi(x,y,z) (z being ncol), matrices, internal function, a for loop, a while loop and rbind.
permTest14 <- function(test) {
  perm14 <- function(x) {
    w <- matrix(seq(strsplit(x, "")[[1]]), ncol = nchar(x))
    rows <- sapply(length(w[1,]), function(z) {for (i in 1:(z-2)) z <- z + z * i; z})
    main <- function(s) {    
      ...
      w <<- matrix(rbind(w, tmp[ipi.duprow.matrixi(tmp[ipi.duprow.matrixi(tmp, w, ncol(s)),], w, ncol(s)),]), ncol = ncol(s))
      ln <<- ln + 1
    }
    ln <- 1
    while (nrow(w) < rows) {
  ...
  tryCatch({
    desc <- paste("Using logic from my Python App, ipi.duprow.matrixi(x,y,z) (z being ncol), matrices, internal function, a for loop, ", 
                  "a while loop and rbind.")
    print(desc)
    stm <- system.time(w <- perm14(test))
    cat(paste(capture.output(stm), "\n"))
    chk <- list(unique.rows = nrow(unique(w)), total.rows = nrow(w), characters = ncol(w))
    TestResult[nrow(TestResult),-(c(1:3,11))] <<- c(desc, chk[2], chk[1], chk[3], round(unname(stm)[1:3],2))
  }, 
...
permTest15 <- function(test) {
  perm15 <- function(x) {
    ...
    main <- function(s) {    
      tmp <- w[0,]
      for (i in seq(1:(ncol(s) - 1))) {
        tmp <- matrix(as.matrix(append(s[, -i], s[, i])), ncol = ncol(s))
        w <<- matrix(rbind(w, tmp[ipi.duprow.matrixi(tmp, w, ncol(s)),]), ncol = ncol(s))
      }
      ln <<- ln + 1
    }
    ln <- 1
...
# Using logic from my Python App, ipi.dup.matrixi(x,y,z) (z being ncol), matrices, internal function, a for loop, a while loop and rbind.
permTest16 <- function(test) {
  perm16 <- function(x) {
    w <- matrix(seq(strsplit(x, "")[[1]]), ncol = nchar(x))
    rows <- sapply(length(w[1,]), function(z) {for (i in 1:(z-2)) z <- z + z * i; z})
    main <- function(s) {    
      tmp <- w[0,]
      for (i in seq(1:(ncol(s) - 1))) {
        if (!ipi.dup.matrixi(append(s[, -i], s[, i]), w, ncol(s))){
          w <<- matrix(rbind(w, append(s[, -i], s[, i])), ncol = ncol(s))
        }
      }
      ln <<- ln + 1
    }
...
permTest17 <- function(test) {
  perm17 <- function(x) {
    ...
    main <- function(s) {    
      tmp <- w[0,]
      for (i in seq(1:(ncol(s) - 1))) {
        if (!dup(append(s[, -i], s[, i]), w, ncol(s))){
          w <<- matrix(rbind(w, append(s[, -i], s[, i])), ncol = ncol(s))
        }
      }
      ln <<- ln + 1
...
# Using logic from my Python App, duplicated, matrices, internal function, a for loop, a while loop and rbind.
permTest18 <- function(test) {
  perm18 <- function(x) {
    w <- matrix(seq(strsplit(x, "")[[1]]), ncol = nchar(x))
    rows <- sapply(length(w[1,]), function(z) {for (i in 1:(z-2)) z <- z + z * i; z})
    main <- function(s) {    
      ...
      w <<- matrix(w[!duplicated(w),], ncol = ncol(s))
      ln <<- ln + 1
    }
    ln <- 1
    while (nrow(w) < rows) {
  ...
  tryCatch({
    desc <- paste("Using logic from my Python App, duplicated, matrices, internal function, a for loop, ", 
                  "a while loop and rbind.")
    print(desc)
    stm <- system.time(w <- perm18(test))
    cat(paste(capture.output(stm), "\n"))
    chk <- list(unique.rows = nrow(unique(w)), total.rows = nrow(w), characters = ncol(w))
    TestResult[nrow(TestResult),-(c(1:3,11))] <<- c(desc, chk[2], chk[1], chk[3], round(unname(stm)[1:3],2))
  }, 
...
permTest19 <- function(test) {
  perm19 <- function(x) {
    ...
    main <- function(s) {    
      ...
      w <- matrix(rbind(w, tmp), ncol = ncol(s))
      w <<- matrix(w[!duplicated(w),], ncol = ncol(s))
      ln <<- ln + 1
    }
    ln <- 1
...
# Using logic from my Python App, ipi.duprem.matrix, matrices, internal function, a for loop, a while loop and rbind.
permTest20 <- function(test) {
  perm20 <- function(x) {
    w <- matrix(seq(strsplit(x, "")[[1]]), ncol = nchar(x))
    rows <- sapply(length(w[1,]), function(z) {for (i in 1:(z-2)) z <- z + z * i; z})
    main <- function(s) {    
    ...
    while (nrow(w) < rows) {
      ...
      rem <- ipi.duprem.matrix(w, nchar(x))
      if (!is.null(rem)){
        w <- matrix(w[-rem,], ncol = nchar(x))
      }
      assign("status", paste("Iteration:", i, "rows:",nrow(w)), envir = .GlobalEnv)
      i <- i + 1
    }
    w
  }
  tryCatch({
    desc <- paste("Using logic from my Python App, ipi.duprem.matrix, matrices, internal function, ", 
                  "a for loop, a while loop and rbind.")
    print(desc)
    stm <- system.time(w <- perm20(test))
    cat(paste(capture.output(stm), "\n"))
    chk <- list(unique.rows = nrow(unique(w)), total.rows = nrow(w), characters = ncol(w))
    TestResult[nrow(TestResult),-(c(1:3,11))] <<- c(desc, chk[2], chk[1], chk[3], round(unname(stm)[1:3],2))
  }, 
...
# Using logic from my Python App, ipi.dupfac.matrix, anyduplicated, matrices, internal function, a for loop, a while loop and rbind.
permTest21 <- function(test) {
  perm21 <- function(x) {
    w <- matrix(seq(strsplit(x, "")[[1]]), ncol = nchar(x))
    rows <- sapply(length(w[1,]), function(z) {for (i in 1:(z-2)) z <- z + z * i; z})
    main <- function(s) {    
    ...
    while(nrow(w) < rows){
      ...
      if (anyDuplicated.matrix(w)){
        w <- ipi.dupfac.matrix(w, ncol = nchar(x))
      }
    }
    w 
  }
  tryCatch({
    desc <- paste("Using logic from my Python App, ipi.dupfac.matrix, anyduplicated, matrices, internal function, a for loop, ", 
                  "a while loop and rbind.")
    print(desc)
    stm <- system.time(w <- perm21(test))
    cat(paste(capture.output(stm), "\n"))
    chk <- list(unique.rows = nrow(unique(w)), total.rows = nrow(w), characters = ncol(w))
    TestResult[nrow(TestResult),-(c(1:3,11))] <<- c(desc, chk[2], chk[1], chk[3], round(unname(stm)[1:3],2))
  }, 
...
# Using logic from my Python App, ipi.dup.rem.old, matrices, internal function, a for loop, a while loop and rbind.
permTest22 <- function(test) {
  perm22 <- function(x) {
    w <- matrix(seq(strsplit(x, "")[[1]]), ncol = nchar(x))
    rows <- sapply(length(w[1,]), function(z) {for (i in 1:(z-2)) z <- z + z * i; z})
    main <- function(s) {    
  ...
  tryCatch({
    desc <- paste("Using logic from my Python App, ipi.dup.rem.old, matrices, internal function, ", 
                  "a for loop, a while loop and rbind.")
    print(desc)
    stm <- system.time(w <- perm22(test))
    cat(paste(capture.output(stm), "\n"))
    chk <- list(unique.rows = nrow(unique(w)), total.rows = nrow(w), characters = ncol(w))
    TestResult[nrow(TestResult),-(c(1:3,11))] <<- c(desc, chk[2], chk[1], chk[3], round(unname(stm)[1:3],2))
  }, 
...
# Using logic from my Python App, more stable Rcpp::cppFunction, matrices, internal function, a for loop, a while loop and rbind.
permTest23 <- function(test) {
  perm23 <- function(x) {
    w <- matrix(seq(strsplit(x, "")[[1]]), ncol = nchar(x))
    rows <- sapply(length(w[1,]), function(z) {for (i in 1:(z-2)) z <- z + z * i; z})
    main <- function(s) {    
    ...
    while (nrow(w) < rows) {
      ...
      while (nrow(w) > rows){
        rem <- dup_rem(apply(w, 1, paste, collapse = ""), bgn)
        if(sum(rem) > 0) {
          w <- matrix(w[-rem,], ncol = nchar(x))
          bgn <- nrow(w)
        } else {
  ...
  tryCatch({
    desc <- paste("Using logic from my Python App, more stable Rcpp::cppFunction, matrices, internal function, ", 
                  "a for loop, a while loop and rbind.")
    print(desc)
    stm <- system.time(w <- perm23(test))
    cat(paste(capture.output(stm), "\n"))
    chk <- list(unique.rows = nrow(unique(w)), total.rows = nrow(w), characters = ncol(w))
    TestResult[nrow(TestResult),-(c(1:3,11))] <<- c(desc, chk[2], chk[1], chk[3], round(unname(stm)[1:3],2))
  }, 
...
RunTests <- function(tests, funcs) {
    ...
    rm(
      list = ls(envir = .GlobalEnv)[
        which(!ls(envir = .GlobalEnv) %in% c(
          "ipi.cleanUp", "RunTests" , "ipi.function.load"
        ))
        ], envir = .GlobalEnv
    )
    #library(IPI)
  ...
  ipi.function.load <- function(fPth , func, envir = .GlobalEnv) {
    ...
    for (i in seq(flBody[[2]])) {
      ...
      error = function(cond) {
        if (as.character(flBody[[2]][i][[1]]) == func){
          cat("Result:", capture.output(as.character(flBody[[2]][i][[1]])), " is a Function call not a function!\nError:",
              capture.output(cond), "\n")
        }
      }
      )
  ...
  req <- list(
    c(fPth = "~/Studies/R/IPI1/Keep_Sakes.R", func = "ipi.duprow.int.matrixi"), 
    c(fPth = "~/Studies/R/IPI1/Keep_Sakes.R", func = "ipi.duprow.matrixi"), 
    c(fPth = "~/Studies/R/IPI1/Keep_Sakes.R", func = "ipi.dup.matrixi"), 
    c(fPth = "~/Studies/R/IPI1/Keep_Sakes.R", func = "ipi.duprem.matrix"), 
    c(fPth = "~/Studies/R/IPI1/Keep_Sakes.R", func = "ipi.dupfac.matrix"),
    c(fPth = "~/Studies/R/IPI1/Keep_Sakes.R", func = "ipi.dup.rem.old")
  )
  if (length(req) > 0) {
    for (i in req) {
      if (!i["func"] %in% ls()) {
        ipi.function.load(i["fPth"], i["func"])
      }
    }
  }
  
  tPth = "~/Studies/R/IPI1/Archive_PermTests.R"
  ln <- 1
  for (i in funcs) {
    for (test in tests){
      tNm <- paste0("permTest",i)
      if (!tNm %in% ls()) {
        ipi.function.load(tPth, tNm)
      }
