...
# Uses c++ and the Rcpp library to check integer matrix for duplicates and returns row numbers
ipi.duprow.int.matrix <-function(x, y) {
  ...
  backlin(as.matrix(x, ncol = ncol(x)), as.matrix(y, ncol = ncol(y)))
}

# Uses c++ and the Rcpp library to check character matrix for duplicates and returns row numbers
ipi.duprow.char.matrix <- function(x, y){
...
ipi.dup.int.matrix <- function(x, y){
